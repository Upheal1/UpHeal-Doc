...
**Algorithmic Bias and Fairness.** The emotion classifier is trained primarily on English-language datasets, which introduces bias against dialectal variations and code-switching common in MENA populations. The system mitigates this by flagging low-confidence classifications (below 60%) and defaulting to a neutral therapeutic response rather than a potentially biased emotion-adaptive one. The community feature employs content moderation through user reporting, but automated bias detection in community recommendations is not yet implemented and remains a limitation.
